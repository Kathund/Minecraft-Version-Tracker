...
import { displayBigMessage, getNestedValue } from './utils/miscUtils.js';
...
class ConfigManager {
  ...
  static async getConfigVersion(): Promise<number> {
    ...
    if (typeof version !== 'number') throw new MinecraftVersionTrackerError('Config Version must be a number.');
    return version;
  }
  ...
  static async backupConfig(config: JsonObject | Config, skipCheck: boolean = false): Promise<void> {
    if (skipCheck === false && getNestedValue(config, 'other.backupConfigs') === false) {
      return console.warn('Config backup is disabled');
    }
    await mkdir('./data/backup/config', { recursive: true });
    await writeFile(
      `./data/backup/config/config_${new Date().toISOString()}.json`,
  ...
  static getLatestVersion(): number {
    return Math.max(1, ...Object.keys(ConfigManager.versions).map(Number));
  }
  ...
  private async migrate() {
    ...
    if (typeof configuredVersion !== 'number') {
      throw new MinecraftVersionTrackerError('Config Version must be a number.');
    }
    ...
    while (currentVersion < latestVersion) {
      ...
      if (!migration) throw new MinecraftVersionTrackerError(`Missing migration for config version ${nextVersion}`);
  ...
  private applyMigration(config: JsonObject, migration: MigrationMap): void {
    for (const [oldPath, rule] of Object.entries(migration)) {
      ...
      if (!this.isJsonValue(value)) {
        throw new MinecraftVersionTrackerError(`Migration value at "${oldPath}" is not valid JSON.`);
      }
      switch (rule.change) {
        case ConfigChangeType.Move: {
          if (!rule.key) throw new MinecraftVersionTrackerError(`Move migration missing target key for "${oldPath}"`);
        ...
        case ConfigChangeType.Transform: {
          if (!rule.transform) {
            throw new MinecraftVersionTrackerError(`Transform migration missing transform function for "${oldPath}"`);
          }
          if (!rule.key) {
            throw new MinecraftVersionTrackerError(`Transform migration missing target key for "${oldPath}"`);
          }
  ...
  private setNestedValue(obj: JsonObject, path: string, value: JsonValue): void {
    ...
    if (!lastKey) throw new MinecraftVersionTrackerError('Cannot set an empty configuration path.');
    ...
    for (const key of keys) {
      ...
      if (!this.isObject(next)) {
        throw new MinecraftVersionTrackerError(`Unable to create configuration path "${path}".`);
      }
      current = next;
    }
  ...
  private static parseJsonObject(input: string, source: string): JsonObject {
    ...
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      throw new MinecraftVersionTrackerError(`${source} must contain a JSON object.`);
    }
  ...
  static async validateExampleConfig(): Promise<Config> {
    ...
    if (config.configVersion !== latestVersion) {
      console.error(
        `Invalid config version... Current versions is ${config.configVersion} and latest is ${latestVersion}`
      );
      process.exit(1);
    }
